<template>
	<div >
		...
		<el-button type="success"  plain @click="add">添加品牌</el-button>
		<el-table :data="tabledata.records" column style="margin-top: 5px;">
			<el-table-column label="品牌名称" show-overflow-tooltip prop="name">
				<template slot-scope="scope">
					<el-popover>
						<span slot="reference">{{scope.row.name}}</span>
						<img width="150"  :src="scope.row.logo"/>
					</el-popover>
				</template>
			</el-table-column>
			<el-table-column label="品牌说明" show-overflow-tooltip prop="description"></el-table-column>
			...
			<el-table-column label="操作"  width="180">
				<template slot-scope="scope" >
						...
						<template v-if="scope.row.active">
							...
							<el-popconfirm
							  :title="'是否将品牌'+scope.row.name+'设置为禁用状态'"
						...
						<template v-else>
							<el-popconfirm
							  :title="'是否将品牌'+scope.row.name+'设置为启用状态'"
